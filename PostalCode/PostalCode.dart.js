...
//  ********** Library PostalCode **************
// ********** Code for PostalCode **************
function PostalCode(_place) {
  this._place = _place;
}
PostalCode.prototype.get$placeName = function() {
  return this._place.$index("placeName");
}
PostalCode.prototype.get$postalCode = function() {
  return this._place.$index("postalcode");
}
PostalCode.prototype.get$lng = function() {
  return this._place.$index("lng");
}
PostalCode.prototype.get$lat = function() {
  return this._place.$index("lat");
}
PostalCode.prototype.toString = function() {
  return ("" + this.get$postalCode() + ", " + this.get$placeName() + ", [" + this.get$lng() + ", " + this.get$lat() + "]");
}
// ********** Code for GeonameClient **************
function GeonameClient(napSelector, dataSelector) {
  this._nap = null;
  this._data = null;
  this._nap = get$$document().query(napSelector);
  this._data = get$$document().query(dataSelector);
}
GeonameClient.prototype.run = function() {
  var $this = this; // closure support
  this._nap.get$on().get$keyUp().add(function _(event) {
    print$("DEBUG: keyUp event");
    if ($this._nap.value.length > (3)) {
      var request = _XMLHttpRequestFactoryProvider.XMLHttpRequest$factory();
      print$(("DEBUG: Searching for " + $this._nap.value));
      var url = ("http://api.geonames.org/postalCodeLookupJSON?postalcode=" + $this._nap.value + "&country=CH&username=chiodonia");
      print$(("DEBUG: API call " + url));
      request.open("GET", url);
      request.get$on().get$load().add((function (event) {
        $this.resetData();
        print$(("DEBUG: " + request.responseText));
        var result = json_JSON.parse(request.responseText);
        print$(("DEBUG: " + result.$index("postalcodes").get$length()));
        print$(("DEBUG: " + result.$index("postalcodes")));
        result.$index("postalcodes").forEach((function (postalCode) {
          print$(("DEBUG: " + postalCode));
          $this.displayPlace(new PostalCode(postalCode));
        })
        );
      })
      , false);
...
GeonameClient.prototype.displayPlace = function(place) {
  var inner = _ElementFactoryProvider.Element$tag$factory("li");
  inner.set$innerHTML(place.toString());
  this._data.get$elements().add(inner);
}
GeonameClient.prototype.resetData = function() {
  this._data.get$nodes().clear$_();
}
// ********** Code for top level **************
function main() {
